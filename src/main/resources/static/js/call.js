...
var CallManager = {
        ...
        this.stompClient = new StompJs.Client({
            ...
            onConnect: function () {
                self.stompClient.subscribe('/user/queue/call', function (message) {
                    self.onSignalReceived(JSON.parse(message.body));
                });
            },
    ...
    onSignalReceived: function (message) {
        ...
        switch (message.type) {
            case 'OFFER': this.onOfferReceived(message); break;
            case 'ANSWER': this.onAnswerReceived(message); break;
            case 'ICE_CANDIDATE': this.onIceCandidateReceived(message); break;
            case 'CALL_REJECT': this.onCallRejected(); break;
            case 'CALL_END': this.cleanup(); break;
        }
    },
    ...
    startCall: function (type) {
        ...
        this.getMediaStream(type)
            .then(function (stream) { self.setupLocalMedia(stream); })
            .then(function () { return self.peerConnection.createOffer(); })
            .then(function (offer) { return self.peerConnection.setLocalDescription(offer); })
            .then(function () { self.sendSignal('OFFER', JSON.stringify(self.peerConnection.localDescription)); })
            .catch(function (err) { self.onMediaError(err); });
    },
    ...
    acceptCall: function () {
        ...
        this.getMediaStream(this.callType)
            .then(function (stream) { self.setupLocalMedia(stream); })
            .then(function () {
                return self.peerConnection.setRemoteDescription(new RTCSessionDescription(window.pendingOffer));
            })
            .then(function () { self.onRemoteDescriptionSet(); })
            .then(function () { return self.peerConnection.createAnswer(); })
            .then(function (answer) { return self.peerConnection.setLocalDescription(answer); })
            .then(function () {
                self.sendSignal('ANSWER', JSON.stringify(self.peerConnection.localDescription));
    ...
    onAnswerReceived: function (message) {
        ...
        this.peerConnection.setRemoteDescription(new RTCSessionDescription(JSON.parse(message.payload)))
            .then(function () { self.onRemoteDescriptionSet(); })
            .then(function () { UI.showActiveCallUI(self.callType); });
    },
    ...
    toggleMic: function () {
        ...
        this.localStream.getAudioTracks().forEach(function (track) { track.enabled = this.micEnabled; }.bind(this));
        document.getElementById('toggleMicBtn').textContent = this.micEnabled ? '🎤 Tắt mic' : '🎤 Bật mic';
    },

    toggleCam: function () {
        if (!this.localStream) return;
        this.camEnabled = !this.camEnabled;
        this.localStream.getVideoTracks().forEach(function (track) { track.enabled = this.camEnabled; }.bind(this));
        document.getElementById('toggleCamBtn').textContent = this.camEnabled ? '📷 Tắt cam' : '📷 Bật cam';
    },
